...
function penci_getPostLikeLink( $post_id ) {
	...
	if ( penci_AlreadyLiked( $post_id ) ) {
		...
		$title = __( 'Unlike', 'pencidesign' );
	}
	else {
		$class = '';
		$title = __( 'Like', 'pencidesign' );
	}
	$return = '<a href="#" class="penci-post-like' . $class . '" data-post_id="' . $post_id . '" title="' . $title . '" data-like="' . __( 'Like', 'pencidesign' ) . '" data-unlike="' . __( 'Unlike', 'pencidesign' ) . '"><i class="fa fa-heart-o"></i></a>';
