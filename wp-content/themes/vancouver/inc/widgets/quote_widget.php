...
add_action( 'widgets_init', 'pencidesign_quote_load_widget' );

function pencidesign_quote_load_widget() {
	register_widget( 'pencidesign_quote_widget' );
}

class pencidesign_quote_widget extends WP_Widget {

	/**
	 * Widget setup.
	 */
	function pencidesign_quote_widget() {
		/* Widget settings. */
		$widget_ops = array( 'classname'   => 'pencidesign_quote_widget', 'description' => __( 'A widget that displays an quote widget on your sidebar', 'pencidesign' ) );

		/* Widget control settings. */
		$control_ops = array( 'id_base' => 'pencidesign_quote_widget' );

		/* Create the widget. */
		global $wp_version;
		if( 4.3 > $wp_version ) {
			$this->WP_Widget( 'pencidesign_quote_widget', __( 'Vancouver Quote', 'pencidesign' ), $widget_ops, $control_ops );
		} else {
			parent::__construct( 'pencidesign_quote_widget', __( 'Vancouver Quote', 'pencidesign' ), $widget_ops, $control_ops );
		}
	}
		...
		?>
			...
			<?php if ( $description ) : ?>
				<div class="quote-widget">
					...
					<p><?php echo wp_kses( $description, '' ); ?></p>
				</div>
			<?php endif; ?>
