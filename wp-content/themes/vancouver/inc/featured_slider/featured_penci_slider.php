...
<div class="featured-area">
	<div class="penci-slider penci-loading <?php echo esc_attr( $featured_style . $class_fix_height ); ?>" data-auto="<?php echo esc_attr( $data_auto ); ?>" data-autotime="<?php echo esc_attr( $auto_time ); ?>" data-speed="<?php echo esc_attr( $auto_speed ); ?>"<?php echo $data_height; ?>>
		<?php
		...
		<ul class="slides">
			<?php if ( $feat_query->have_posts() ) : while ( $feat_query->have_posts() ) : $feat_query->the_post();
				...
				if ( ! empty( $button_text ) ) {
					...
					if ( ! empty( $button_url ) ):
						$button_html = '<div class="penci-button"><a class="pencislider-button"'.$style_button.' href="' . $button_url . '">' . $button_text . '</a></div>';
					endif;
				}
						...
						<div class="pencislider-container penci-<?php echo $animation?> align-<?php echo $slider_align; ?>">
							<div class="pencislider-content">
								<?php if( !empty( $slider_title ) ): ?>
									<h2 class="pencislider-title"<?php echo $title_style; ?>"><?php echo $slider_title; ?></h2>
								<?php endif; ?>
								<?php if( !empty( $caption ) ): ?>
									<p class="pencislider-caption"<?php echo $caption_style; ?>"><?php echo $caption; ?></p>
								<?php endif; ?>
